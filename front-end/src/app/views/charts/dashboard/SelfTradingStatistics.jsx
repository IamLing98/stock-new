...
const SelfTradingStatistics = (props) => {
    ...
    const crawlerData = () => {
        //Set data buy
        fetch('https://finfo-api.vndirect.com.vn/v4/proprietary_trading?q=type:STOCK,ETF,IFC&sort=date:desc&size=3000')
            .then((res) => res.json())
            .then((json) => {
                if (json != null) {
    ...
    const processData = (chartID) => {
        ...
        switch (chartID) {
            case 'TDGTGD':
                const dsDate = [...new Set(dataChart.map((val) => val.date))];
                dsDate?.forEach((label) => {
                    var obj = { label: label, value: 0 };
                    var dataOfDate = dataChart.filter((data) => data.date === label);
                ...
                break;
            case 'TDKLGD':
                const dsDate2 = [...new Set(dataChart.map((val) => val.date))];
    
                dsDate2?.forEach((label) => {
                    var obj = { label: label, value: 0 };
                    var dataOfDate = dataChart.filter((data) => data.date === label);
                ...
                break;
            default:
                const dsDateDefault = [...new Set(dataChart.map((val) => val.date))];
...
export default SelfTradingStatistics;
